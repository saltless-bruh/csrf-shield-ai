...
# Maximum possible static severity (all 11 rules at CRITICAL).
MAX_POSSIBLE_SEVERITY: float = 11 * 1.0
...
class RiskLevel(Enum):
    """Risk classification levels (FR-402, PROPOSAL §10.2)."""
...
def detect_context_modifiers(
    ...
    if any(p in path for p in _FINANCIAL_PATTERNS):
        modifiers.append(("Financial data endpoint", 15))

    # +10: Modifies user data.
    if any(p in path for p in _USER_DATA_PATTERNS):
        modifiers.append(("Modifies user data", 10))

    # +10: Admin-only endpoint.
    if any(p in path for p in _ADMIN_PATTERNS):
        modifiers.append(("Admin-only endpoint", 10))

    # -5: Uses HTTPS.
    parsed = urlparse(url)
    if parsed.scheme == "https":
        modifiers.append(("Uses HTTPS", -5))
    ...
    if protection_count >= 2:
        modifiers.append(("Multiple CSRF protections", -15))
    ...
    if http_method.upper() == "GET":
        ...
        if any(p in query for p in action_params):
            modifiers.append(("GET-based state change", 20))
